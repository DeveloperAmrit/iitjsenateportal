...
// Placeholder for social media icons if you don't have SVGs
...
// --- NEW ADDITIONS FOR AQI HEART ---

// AQI Thresholds and corresponding Tailwind CSS colors
// These are common approximate ranges for PM2.5 (µg/m³) but can be adjusted based on specific standards (e.g., CPCB).
const getAqiStatusAndColor = (pm2_5: number | null) => {
  if (pm2_5 === null || isNaN(pm2_5)) {
    return { status: "N/A", color: "text-gray-500", aqiHeartColor: "#9CA3AF" }; // Gray
  } else if (pm2_5 <= 12.0) { // Good (WHO annual mean guideline is 5 µg/m³, daily 15 µg/m³)
    return { status: "Good", color: "text-green-400", aqiHeartColor: "#4ADE80" }; // Green
  } else if (pm2_5 <= 35.4) { // Moderate
    return { status: "Moderate", color: "text-yellow-400", aqiHeartColor: "#FACC15" }; // Yellow
  } else if (pm2_5 <= 55.4) { // Unhealthy for Sensitive Groups
    return { status: "Unhealthy for Sensitive Groups", color: "text-orange-400", aqiHeartColor: "#FB923C" }; // Orange
  } else if (pm2_5 <= 150.4) { // Unhealthy
    return { status: "Unhealthy", color: "text-red-500", aqiHeartColor: "#EF4444" }; // Red
  } else { // Very Unhealthy / Hazardous
    return { status: "Hazardous", color: "text-purple-500", aqiHeartColor: "#A855F7" }; // Purple
  }
};

// Beating Heart SVG Component
const BeatingHeart: React.FC<{ color: string }> = ({ color }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24"
    fill={color}
    className="w-5 h-5 animate-pulse-fast inline-block ml-1" // Apply custom animation class
    style={{ color: color }} // Ensure fill color from prop
  >
    <path
      fillRule="evenodd"
...
const Footer: React.FC = () => {
  const [weatherData, setWeatherData] = useState<{
    ...
    humidity: string;
    wind: string;
    pm2_5_value: number | null; // Store as number for comparison
    aqi_status: string; // "Good", "Moderate", etc.
    aqi_color_class: string; // Tailwind class like "text-green-400"
    aqi_heart_color: string; // Hex color for SVG fill
    time: string;
    date: string;
    icon: string;
  } | null>(null);
    ...
    const fetchWeatherAndAQI = async () => {
      try {
        ...
        if (weatherJson.current) {
          ...
          const pm2_5 = aqiJson?.current?.pm2_5 ?? null; // Get PM2.5 value, null if not available

          const { status, color, aqiHeartColor } = getAqiStatusAndColor(pm2_5); // Determine AQI status and colors
          ...
          setWeatherData({
            ...
            pm2_5_value: pm2_5, // Store numeric value
            aqi_status: status,
    ...
    <footer className="w-full bg-gray-900 text-gray-100 pt-12 pb-6 px-4 md:px-8 lg:px-12 font-sans">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-y-10 gap-x-8">
        ...
        <div className="flex flex-col items-center md:items-start text-center md:text-left">
          <Image
            src="/images/IITJ/logo/iitjlogo.png" // Replace with your actual IITJ logo path
        ...
        <div className="flex flex-col items-center md:items-start text-center md:text-left">
          ...
          <ul className="space-y-2 text-gray-300 text-base">
            ...
            <li>
                <button
                    onClick={() => alert("Dark/Light Mode Toggle functionality goes here!")} // Placeholder for dark/light mode toggle
        ...
        <div className="flex flex-col items-center md:items-start text-center md:text-left">
          ...
          {weatherData ? (
            <div className="space-y-2">
              ...
              <p className="text-2xl font-bold text-yellow-300">{weatherData.temp}</p> {/* Highlight temperature */}
